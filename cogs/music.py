...
class TutorialButton(discord.ui.View):
    def __init__(self):
        ...
        botaourl = discord.ui.Button(
            label="Código no GitHub", url="https://github.com/vncsmnl")
...
class music(commands.Cog):
    ...
    def play_next(self):
        if len(self.music_queue) > 0:
            ...
            # Check if looping is enabled before removing the first element
            if self.is_looping:
    ...
    @app_commands.command(name="loop", description="Ativa o loop da música atual.")
    async def toggle_loop(self, interaction: discord.Interaction):
        ...
        embedvc = discord.Embed(
            colour=32768 if self.is_looping else 12255232,
            description=f"Looping {'ativado' if self.is_looping else 'desativado'}."
        )
        await interaction.followup.send(embed=embedvc)
